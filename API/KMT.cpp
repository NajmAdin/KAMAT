...
string KMT::RegularDataType(string TY)
{
	if (TY.size() < 3)
	{
		return "non";
...
bool KMT::IfTableExist(string name)
{
	ifstream dbFile(mainDirctory + "/" + CurntlyDB + "/" + CurntlyDB + ".db.km");
	int num;
	dbFile >> num;
...
string KMT::inputDataType(string input, string Type)
{
...
int KMT::INSERT(string Table, vector<string> v)
{
...
int KMT::SELECT(string Table, vector<vector<string>> &ResultData, string condition, string printerChk)
{
	// SELECT * FROM Customers;
	for (int i = 0; i < Table.size(); i++)
	{
		Table += tolower(Table[i]);
	}
	if (!IfTableExist(Table))
	{
		return 4;
...
int KMT::DEL(string Table, string condition, string printerChk)
{
	for (int i = 0; i < Table.size(); i++)
	{
...
bool KMT::DeleteSubTable(string table, string db)
{
	string str = " ";
	char *ch = &str[0];
	for (int i = 0; i < 101; i++)
...
int KMT::DROP(string Type, string name)
{
...
int KMT::Create_Table(string Name, vector<pair<string, string>> ColumnAndType, unsigned int primKeyIdx)
{
